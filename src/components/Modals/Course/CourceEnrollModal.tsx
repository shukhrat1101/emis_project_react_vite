// src/components/Modals/Course/CourceEnrollModal.tsx
...
import "../Modal.scss";
import "../Competition/ParticipantModal.scss";
...
const CourceEnrollModal: React.FC<Props> = ({ isOpen, onClose, courseId, onSaved }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canSave) return toast.warning("Avval JSHSHIRni tasdiqlang.");
    try {
  ...
  return (
    <div className="pp-modal-overlay">
      <div className="pp-modal">
        <h3 className="pp-title">Tinglovchi qo‘shish</h3>

        <form className="pp-form" onSubmit={handleSubmit}>
          <div className="pp-input-group">
            <label htmlFor="crs-pinfl">JSHSHIR (14 raqam)</label>
            ...
            {exists === true && (
              <div className="pp-hint pp-hint-ok">{hint || "Ushbu PINFL tizimda mavjud."}</div>
            )}
            {exists === false && (
              <>
                <div className="pp-hint pp-hint-warn">
                  {hint || "Ushbu PINFL bo‘yicha xodim topilmadi."}
                </div>
                <div className="pp-actions-inline">
                  ...
                  >
                    <FaUserPlus /> Harbiy xizmatchi qo‘shish
                  </button>
                </div>
              </>
            )}
...
export default CourceEnrollModal;
